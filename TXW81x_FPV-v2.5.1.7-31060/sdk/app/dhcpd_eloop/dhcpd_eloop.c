...
__init static int32 _dhcpd_start(char *ifname, struct dhcpd_param *param)
{
    ...
    eloop_add_fd( dhcpd->sock, EVENT_READ, EVENT_F_ENABLED, dhcpd_task_eloop, NULL );
    eloop_add_timer(500, EVENT_F_ENABLED, dhcpd_check_expired, NULL);
    return RET_OK;
}
...
int32 dhcpd_start_eloop(char *ifname, struct dhcpd_param *param)
{
    ...
    if(dhcpd){
        dhcpd->stop = 0;
        dhcpd_err("dhcpd already running ...\r\n");
        return RET_OK;
    }
