...
static uint32_t sd_tmp_buf[SD_TMP_MAX_SIZE/4];
int no_frame_record_video_psram(void *fp,void *d,int flen)
{
    ...
    LL_FOREACH_SAFE(dest_list,el,tmp)
    {
		...
		while(fp && total_len && uint_len_tmp)
        {
			...
			hw_memcpy0(sd_tmp_buf,jpeg_buf_addr+offset,write_len);
			
            osal_fwrite(sd_tmp_buf,1,write_len,fp);
...
void sd_save_thread(struct sd_save_s *ctrl)
{
    ...
    while(sd_save_flag)
    {
        ...
        err = new_sd_save_avi2(&sd_save_flag,s,audio_s,ctrl->fps,ctrl->frq,60*1,640,480);
		if(err == -2)
		{
...
static struct os_task     sd_save_task;
...
void start_record_thread(uint8_t video_fps,uint8_t audio_frq)
{
	if(!record_thread_status)
	{
		...
		OS_TASK_INIT("sd_save", &sd_save_task, sd_save_thread, (uint32)&g_sd_save_msg, OS_TASK_PRIORITY_NORMAL, 1024);  
	} 
}
