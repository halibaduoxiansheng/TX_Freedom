...
static void udp_tx_start_listen(struct event *ei, void *d)
{
	...
	struct sockaddr remote_addr;
	int nRecev;
	nRecev = recvfrom(t_socket, (char*)speedTest_buf, sizeof(speedTest_buf)-1, 0, &remote_addr, &retval);
	if(nRecev > 0)
	{
		speedTest_buf[nRecev] = 0;
		if(strcmp((char*)speedTest_buf,"start") == 0 )
		{
			eloop_set_event_enabled(udp_struct->timer_e,1);
		}
		else if(strcmp((char*)speedTest_buf,"stop") == 0 )
		{
...
static void udp_rx(void *ei, void *d)
{
	int t_socket = (int)d;
	socklen_t retval = sizeof(struct sockaddr_in);
	struct sockaddr remote_addr;
	int nRecev;
	nRecev = recvfrom(t_socket, (char*)speedTest_buf, sizeof(speedTest_buf), 0, &remote_addr, &retval);
	if(nRecev > 0)
	{
		speed_count += nRecev;
	}
