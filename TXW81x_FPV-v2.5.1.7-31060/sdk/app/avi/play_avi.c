...
static stream_ops_func stream_avi_ops = 
{
...
stream* creat_play_avi_stream(const char *name,char *filepath,char *recv_name)
{
    ...
    else
    {
        ...
        if(recv_name)
        {
            streamSrc_bind_streamDest(s,recv_name);
...
uint32_t start_play_avi_thread(stream *s)
{
    ...
    csi_kernel_task_new((k_task_entry_t)play_web_avi_thread, "play_avi", s, 9, 0, NULL, 1024,&handle);
    return 0;
}
...
void play_web_avi_thread(void *d)
{
    ...
    while( src && info)
    {
        ...
        if(base_tmp != base)
        {
			...
			data_s->ref = 0;
			data_s->type = 1;
            //判断是否要进行发送数据,没到时间就要延时
            //os_printf("now:%d\tdiff:%d\n",os_jiffies(),diff);
            while(os_jiffies()<diff)
            {
                os_sleep_ms(1);
            }
            diff = os_jiffies();
