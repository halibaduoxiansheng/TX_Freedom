...
void  avi_read_thread(void *d)
{
	...
	name = d;
	_os_printf("run  %s\r\n",name);
	fp = osal_fopen((const char *)name,"rb");
	if(!fp)
	{
		_os_printf("%s open avi err\n",__FUNCTION__);
		goto avi_read_err;
	}
...
//		res = avidemux_read_begin(str, &base, &size);
		...
		if(res)
		{
			_os_printf("base:%X\tsize:%X\tres:%d\n",base,size,res);
			break;
		}
...
//		jpg_size = avidemux_read(str,addr,size);
		...
		_os_printf("jpgsize:%d\n",jpg_size);

		jpg_analyze(addr,(uint8_t *)photo_size);
		_os_printf("width:%d  high:%d\r\n",photo_size[0],photo_size[1]);
		jpg_decode_to_lcd((uint32)addr,photo_size[1],photo_size[0],320,240);
...
void jpeg_photo_explain(uint8* photo_name){
	...
	_os_printf("width:%d  high:%d\r\n",photo_size[0],photo_size[1]);
	jpg_decode_to_lcd((uint32)data_buf,photo_size[1],photo_size[0],320,240);
...
void rec_playback_thread_init(uint8* rec_name){
	k_task_handle_t rec_task_handle;
	//rec_arg[0] = (w&0xffff) | ((h<<16)&0xffff0000);
	//rec_arg[1] = continuous_spot;
	extern void  lvgl_recv_avi_thread(void *d);
    csi_kernel_task_new((k_task_entry_t)lvgl_recv_avi_thread, "rec_playback_thread", rec_name, 10, 0, NULL, 1024, &rec_task_handle);
}
...
void  lvgl_recv_avi_thread(void *d)
{
	...
	cmd = (uint32_t *)&s->priv;
	play_avi_s = creat_play_avi_stream(S_PLAYBACK,filepath,R_LVGL_JPEG);
	if(!play_avi_s)
	{
		goto lvgl_recv_avi_end;
	}
