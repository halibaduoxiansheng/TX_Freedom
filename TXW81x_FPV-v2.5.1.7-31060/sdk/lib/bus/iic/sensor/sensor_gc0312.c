#include "sys_config.h"
#include "typesdef.h"
#include "lib/video/dvp/cmos_sensor/csi.h"
#include "tx_platform.h"
#include "list.h"
#include "dev.h"
#include "hal/i2c.h"


#if DEV_SENSOR_GC0312


#if 0
#ifdef I4_DEV_EW36_V01
SENSOR_INIT_SECTION static const unsigned char GC0312InitTable[CMOS_INIT_LEN]=
{

#if 1
	0xFE,0xF0,
	0xFE,0xF0,
	0xFE,0x00,
	0xFC,0x0E,
	0xFC,0x0E,
	
	0xF2,0x07,
	0xF3,0x00,
	0xF7,0x1B,
	0xF8,0x04,
	0xF9,0x0E,
	0xFA,0x11,
	
	0x00,0x2F,
	0x01,0x0F,
	0x02,0x04,
	0x03,0x03,
	0x04,0x50,
	
	0x09,0x00,
	0x0A,0x00,
	
	0x0B,0x00,
	0x0C,0x04,
	
	0x0D,0x01,
	0x0E,0xE8,
	
	0x0F,0x02,
	0x10,0x88, 
	0x16,0x00,
	0x17,0x14,	//0x17
	,

	//采耳
	0xFE,0x00,
	0x77,0x36,
	0x78,0x23,
	0x79,0x25,
	0x95,0x80,
	0xD0,0x40,
	0xD1,0x40, 
	0xD2,0x40, 
	0xD3,0x47, //0x47 
	0xD4,0x40, 
	
	0xFE,0x01,
	0x0b,0x02,
	0x13,0x58, //0x65


#endif

	//////////////////////////////////////////////////////////////////////////////////////
	#if 0	//test	 
	0xfe,0x00,	//i4scmd effect 0 42 f9
	//0x42,0xf9,	//ABS\AWB disable（自动白平衡关闭）	
	//
	#if 0	//demo
	//0
	0x77,0x74,
	0x78,0x70, //0x40,
	0x79,0x5f,
	//
	0xd0,0x40,
	0xd1,0x34,
	0xd2,0x34,
	0xd3,0x40,
	0xd4,0x38,
	//
	0x82,0x14 ,
	0x83,0x0b,
	//
	0x95,0x65,
	//1
	0x13,0x78, //0x75,
	#endif 
	
	//
	#if 1
	0x77,0x40,		//i4scmd effect 0 77 40	R
	0x78,0x23,		//i4scmd effect 0 78 2a	G		//30	32
	0x79,0x36,		//i4scmd effect 0 79 36	B
	#endif
	//
	0xd0,0x40,		//i4scmd effect 0 d0 40	Global 		saturation(全局饱和度)
	0xd1,0x42,		//i4scmd effect 0 d1 20	Cb:0x20*1	saturation(饱和)
	0xd2,0x42,		//i4scmd effect 0 d2 40	Cr:0x20*1	saturation(饱和)
	0xd3,0x40,		//i4scmd effect 0 d3 40	0x40*1		Contrast对比度(清晰度/亮度)
	0xd4,0x80,		//i4scmd effect 0 d4 80 对比度中心值（默认0x80）
	//
	0x95,0x88,		//edge enhancement(边缘增强)
	//0x71, 0x24,	//
	//0x72, 0x44,	//
	0x82,0x14,		//亮处去噪，值越大去噪越强
	0x83,0x14,		//暗处去噪，值越大去噪越强
	//
	0xfe,0x01,
	0x13,0x80,		//亮度
	//0x0b,0x02,

	#endif	
	////////////////////////////////////////////////////////////////////////////////////

	-1,-1
};
#endif 

#endif

//==============================================================================
#if 0	//BK->TX
SENSOR_INIT_SECTION static const unsigned char GC0312InitTable[CMOS_INIT_LEN]=
{
	 0xFE,0xF0 ,
	 0xFE,0xF0 ,
	 0xFE,0x00 ,
	 0xFC,0x0E ,
	 0xFC,0x0E ,
	 0xF2,0x07 ,
	 0xF3,0x00 ,
	 0xF7,0x1B ,
	 0xF8,0x04 ,
	 0xF9,0x0E ,
	 0xFA,0x11 ,
	 0x00,0x2F ,
	 0x01,0x0F ,
	 0x02,0x04 ,
	 0x03,0x03 ,
	 0x04,0x50 ,
	 0x09,0x00 ,
	 0x0A,0x00 ,
	 0x0B,0x00 ,
	 0x0C,0x04 ,
	 0x0D,0x01 ,
	 0x0E,0xE8 ,
	 0x0F,0x02 ,
	 0x10,0x88 , 
	 0x16,0x00 ,
	 0x17,0x14 ,	//0x14(youfu_ew37_v007)	0x17,0x17 ,
	 0x18,0x1A ,
	 0x19,0x14 ,
	 0x1B,0x48 ,
	 0x1E,0x6B ,
	 0x1F,0x28 ,
	 0x20,0x8B ,
	 0x21,0x49 ,
	 0x22,0xB0 ,
	 0x23,0x04 ,
	 0x24,0x16 ,
	 0x34,0x20 ,
	 0x26,0x23 ,
	 0x28,0xFF , 
	 0x29,0x00 ,
	 0x33,0x10 ,
	 0x37,0x20 ,
	 0x38,0x10 ,
	 0x47,0x80 ,
	 0x4E,0x66 ,
	 0xA8,0x02 ,
	 0xA9,0x80 ,
	/////////////////////////////////////////////////
	//////////////////	ISP reg   ///////////////////
	/////////////////////////////////////////////////
	0x40,0xff,		//tx
	0x41,0x21,
	0x42,0xcf,
	0x44,0x02,
	0x45,0xa8,
	0x46,0x02,
	0x4a,0x11,
	0x4b,0x01,
	0x4c,0x20,
	0x4d,0x05,
	0x4f,0x01,
	0x50,0x01,
	0x55,0x01,
	0x56,0xe0,
	0x57,0x02,
	0x58,0x80,
	/*
	 0x40,0xFF ,	//bk
	 0x41,0x21 ,
	 0x42,0x4d ,
	 0x44,0x02 ,
	 0x45,0xA8 ,
	 0x46,0x03 ,
	 0x4A,0x11 ,
	 0x4B,0x01 ,
	 0x4C,0x20 ,
	 0x4D,0x05 ,
	 0x4F,0x01 ,
	 0x50,0x01 ,
	 0x55,0x01 ,
	 0x56,0xe0 ,
	 0x57,0x02 ,
	 0x58,0x80 ,
	*/ 
	 0x70,0x70 ,
	 0x5A,0x84 ,
	 0x5B,0xC9 ,
	 0x5C,0xED ,
	 0x77,0x4f ,  
	 0x78,0x40 , 
	 0x79,0x5a ,
	 0x7A,0x80 , 
	 0x7B,0x80 ,  
	 0x7C,0x80 ,  
	 0x82,0x10 ,	//0x18 (denoising)			1F\10\18\
	 0x83,0x10 ,	//0x0b (denoising)			0F\10\0B\
	 0x89,0xF0 ,
	 0x8F,0xAA ,
	 0x90,0x8C ,
	 0x91,0x90 ,
	 0x92,0x03 ,
	 0x93,0x03 ,
	 0x94,0x05 ,
	 0x95,0x34 ,	//0x66 (edge sharpening)	54\34\88\
	 0x96,0xF0 ,	//0xf0 (edge sharpening)	FF\F0\F0\
	  ,
	#if 1
	//75R-lw
	 0xFE,0x00 ,
	 0x77,0x42 ,	// 0x77,0x36 ,
	 0x78,0x23 ,
	 0x79,0x25 ,
	 0x95,0x80 ,
	 0xD0,0x40 ,
	 0xD1,0x40 , 
	 0xD2,0x40 , 
	 0xD3,0x47 , //0x47 
	 0xD4,0x40 , 
 
	 0xFE,0x01 ,
	 0x0b,0x02 ,
	 0x13,0x58 , //0x65
	#endif
	
#if 1
	0xfe,0x03,
	0x01,0x00,
	0x02,0x00,
	0x10,0x00,
	0x15,0x00,
	0xfe,0x00,
#endif
		///////////////////OUTPUT//////////////////////
	0xf3,0xff, // output_enable
	-1,-1

};
#endif
//==============================================================================

#if 0	//TX_DEFAULT
SENSOR_INIT_SECTION static const unsigned char GC0312InitTable[CMOS_INIT_LEN]=
{
#if 1
0xfe,0xf0,
0xfe,0xf0,
0xfe,0x00,
0xfc,0x0e ,
0xfc,0x0e ,
0xf2,0x08 ,
0xf3,0x00,// output_disable 
0xf7,0x1b ,
0xf8,0x03 ,
0xf9,0x0e ,
0xfa,0x11,

#endif

#if 1
	/////////////////////////////////////////////////
	/////////////////  CISCTL reg	/////////////////
	/////////////////////////////////////////////////
0x00,0x2f,
0x01,0x0f,
0x02,0x04,
0x03,0x03,
0x04,0x50,
0x09,0x00,
0x0a,0x00,
0x0b,0x00,
0x0c,0x04,
0x0d,0x01,
0x0e,0xe8,
0x0f,0x02,
0x10,0x88,
0x16,0x00,
0x17,0x17,	//0x14(youfu_ew37_v007)		0x17
0x18,0x1a,
0x19,0x14,
0x1b,0x48,
0x1c,0x1c,
0x1e,0x6b,
0x1f,0x28,
0x20,0x8b,//89 travis 20140801
0x21,0x49,
0x22,0xb0,
0x23,0x04,
0x24,0x16,
0x34,0x20,
	
	/////////////////////////////////////////////////
	////////////////////   BLK	 ////////////////////
	/////////////////////////////////////////////////
0x26,0x23,
0x28,0xff,
0x29,0x00,
0x32,0x00,
0x33,0x10 ,
0x37,0x20,
0x38,0x10,
0x47,0x80,
0x4e,0x66,
0xa8,0x02,
0xa9,0x80,
	
	/////////////////////////////////////////////////
	//////////////////	ISP reg   ///////////////////
	/////////////////////////////////////////////////
0x40,0xff,
0x41,0x21,
0x42,0xcf,
0x44,0x02,
0x45,0xa8 ,
0x46,0x02 ,
0x4a,0x11,
0x4b,0x01,
0x4c,0x20,
0x4d,0x05,
0x4f,0x01,
0x50,0x01,
0x55,0x01,
0x56,0xe0,
0x57,0x02,
0x58,0x80,
	
	/////////////////////////////////////////////////
	///////////////////   GAIN   ////////////////////
	/////////////////////////////////////////////////
0x70,0x70,
0x5a,0x84,
0x5b,0xc9,
0x5c,0xed,
0x77,0x74,
0x78,0x70, //0x40,
0x79,0x5f,
	
	///////////////////////////////////////////////// 
	///////////////////   DNDD  /////////////////////
	///////////////////////////////////////////////// 
0x82,0x14 ,
0x83,0x0b,
0x89,0xf0,
	
	///////////////////////////////////////////////// 
	//////////////////   EEINTP  ////////////////////
	///////////////////////////////////////////////// 
0x8f,0xaa ,
0x90,0x8c ,
0x91,0x90,
0x92,0x03 ,
0x93,0x03 ,
0x94,0x05 ,
0x95,0x65 ,
0x96,0xf0 ,
	
	///////////////////////////////////////////////// 
	/////////////////////  ASDE  ////////////////////
	///////////////////////////////////////////////// 
0xfe,0x00,
                                       
0x9a,0x20,
0x9b,0x80,
0x9c,0x40,
0x9d,0x80,
	                               
0xa1,0x30,
 0xa2,0x32,
0xa4,0x80,//30 travis 20140929
0xa5,0x28,//30 travis 20140929
0xaa,0x30,//10 travis 20140929 				
0xac,0x22,
	 
	/////////////////////////////////////////////////
	///////////////////   GAMMA   ///////////////////
	/////////////////////////////////////////////////
/*0xfe,0x00,//default
0xbf,0x08,
0xc0,0x16,
0xc1,0x28,
0xc2,0x41,
0xc3,0x5a,
0xc4,0x6c,
0xc5,0x7a,
0xc6,0x96,
0xc7,0xac,
0xc8,0xbc,
0xc9,0xc9,
0xca,0xd3,
0xcb,0xdd,
0xcc,0xe5,
0xcd,0xf1,
0xce,0xfa,
0xcf,0xff,*/
////big gammmar///////////////////////////////
 
0xfe,0x00,//big gamma
0xbf,0x08,
0xc0,0x1d,
0xc1,0x34,
0xc2,0x4b,
0xc3,0x60,
0xc4,0x73,
0xc5,0x85,
0xc6,0x9f,
0xc7,0xb5,
0xc8,0xc7,
0xc9,0xd5,
0xca,0xe0,
0xcb,0xe7,
0xcc,0xec,
0xcd,0xf4,
0xce,0xfa,
0xcf,0xff,
	



	/////////////////////////////////////////////////
	///////////////////   YCP  //////////////////////
	/////////////////////////////////////////////////
0xd0,0x40,
0xd1,0x34,
0xd2,0x34,
0xd3,0x40,
0xd6,0xf2,
0xd7,0x1b,
0xd8,0x18,
0xdd,0x03,
	
	/////////////////////////////////////////////////
	////////////////////   AEC   ////////////////////
	/////////////////////////////////////////////////
0xfe,0x01,
0x05,0x30,
0x06,0x75,
0x07,0x40,
0x08,0xb0,
0x0a,0x85, //0xc5,
0x0b,0x11,
0x0c,0x00,
0x12,0x52,
0x13,0x78, //0x75,
0x18,0x67,//0X95
0x19,0x67,//0X96
0x1f,0x20,
0x20,0xc0,
0x3e,0x40,
0x3f,0x57,
0x40,0x7d,
0x03,0x60,
0x44,0x02,
	
	/////////////////////////////////////////////////
	////////////////////   AWB   ////////////////////
	/////////////////////////////////////////////////
0xfe,0x01,
0x1c,0x91,
0x21,0x15,
0x50,0x80,
0x56,0x04,
0x59,0x08,
0x5b,0x02,
0x61,0x8d,
0x62,0xa7,
0x63,0xd0,
0x65,0x06,
0x66,0x06,
0x67,0x84,
0x69,0x08,
0x6a,0x25,
0x6b,0x01,
0x6c,0x00,//00
0x6d,0x02,
0x6e,0xf0,
0x6f,0x80,//80
0x76,0x80,
0x78,0xaf,
0x79,0x75,
0x7a,0x40,
0x7b,0x50,	
0x7c,0x0c,
	

0x90,0xc9,//stable AWB 
0x91,0xbe,
0x92,0xe2,
0x93,0xc9,
0x95,0x1b,
0x96,0xe2,
0x97,0x49,
0x98,0x1b,
0x9a,0x49,
0x9b,0x1b,
0x9c,0xc3,
0x9d,0x49,
0x9f,0xc7,
0xa0,0xc8,
0xa1,0x00,
0xa2,0x00,
0x86,0x00,
0x87,0x00,
0x88,0x00,
0x89,0x00,
0xa4,0xb9,
0xa5,0xa0,
0xa6,0xba,
0xa7,0x92,
0xa9,0xba,
0xaa,0x80,
0xab,0x9d,
0xac,0x7f,
0xae,0xbb,
0xaf,0x9d,
0xb0,0xc8,
0xb1,0x97,
0xb3,0xb7,
0xb4,0x7f,
0xb5,0x00,
0xb6,0x00,
0x8b,0x00,
0x8c,0x00,
0x8d,0x00,
0x8e,0x00,
0x94,0x55,
0x99,0xa6,
0x9e,0xaa,
0xa3,0x0a,
0x8a,0x00,
0xa8,0x55,
0xad,0x55,
0xb2,0x55,
0xb7,0x05,
0x8f,0x00,
0xb8,0xcb,
0xb9,0x9b,	
//default  awb/////////////////////
/*	 
0xa4,0xb9, //default AWB
0xa5,0xa0,
0x90,0xc9, 
0x91,0xbe,                                 
0xa6,0xb8, 
0xa7,0x95, 
0x92,0xe6, 
0x93,0xca,                                
0xa9,0xbc, 
0xaa,0x95, 
0x95,0x23, 
0x96,0xe7,                                  
0xab,0x9d, 
0xac,0x80,
0x97,0x43, 
0x98,0x24,                                
0xae,0xb7, 
0xaf,0x9e, 
0x9a,0x43,
0x9b,0x24,                                 
0xb0,0xc8, 
0xb1,0x97,
0x9c,0xc4, 
0x9d,0x44,                               
0xb3,0xb7, 
0xb4,0x7f,
0x9f,0xc7,
0xa0,0xc8,                               
0xb5,0x00, 
0xb6,0x00,
0xa1,0x00,
0xa2,0x00,                         
0x86,0x60,
0x87,0x08,
0x88,0x00,
0x89,0x00,
0x8b,0xde,
0x8c,0x80,
0x8d,0x00,
0x8e,0x00,                               
0x94,0x55,
0x99,0xa6,
0x9e,0xaa,
0xa3,0x0a,
0x8a,0x0a,
0xa8,0x55,
0xad,0x55,
0xb2,0x55,
0xb7,0x05,
0x8f,0x05,                              
0xb8,0xcc,
0xb9,0x9a,
*/ 
	/////////////////////////////////////////////////
	////////////////////   CC    ////////////////////
	/////////////////////////////////////////////////
0xfe,0x01,
	
0xd0,0x38,//skin red
0xd1,0x00,
0xd2,0x02,
0xd3,0x04,
0xd4,0x38,
0xd5,0x12,	
0xd6,0x30,
0xd7,0x00,
0xd8,0x0a,
0xd9,0x16,
0xda,0x39,
0xdb,0xf8,

	/////////////////////////////////////////////////
	////////////////////   LSC   ////////////////////
	/////////////////////////////////////////////////
0xfe,0x01,
0xc1,0x3c,
0xc2,0x50,
0xc3,0x00,
0xc4,0x40,
0xc5,0x30,
0xc6,0x30,
0xc7,0x10,
0xc8,0x00,
0xc9,0x00,
0xdc,0x20,
0xdd,0x10,
0xdf,0x00,
0xde,0x00,
	
	/////////////////////////////////////////////////
	///////////////////  Histogram	/////////////////
	/////////////////////////////////////////////////
0x01,0x10,
0x0b,0x31,
0x0e,0x50,
0x0f,0x0f,
0x10,0x6e,
0x12,0xa0,
0x15,0x60,
0x16,0x60,
0x17,0xe0,
	
	/////////////////////////////////////////////////
	//////////////	Measure Window	  ///////////////
	/////////////////////////////////////////////////
0xcc,0x0c ,
0xcd,0x10,
0xce,0xa0,
0xcf,0xe6,
	
	/////////////////////////////////////////////////
	/////////////////	dark sun   //////////////////
	/////////////////////////////////////////////////
0x45,0xf7,
0x46,0xff,
0x47,0x15,
0x48,0x03 ,
0x4f,0x60,

	//////////////////banding//////////////////////
0xfe,0x00 ,
0x05,0x02,
0x06,0x2c,//HB
0x07,0x00,
0x08,0x40,//VB
	
0xfe,0x01,
0x25,0x00 ,  //anti-flicker step [11:8]
0x26,0x6a,   //anti-flicker step [7:0]

0x27,0x01, //
0x28,0xe0, 
0x29,0x02,//
0x2a,0xa0,
0x2b,0x03,//
0x2c,0xc0,
0x2d,0x04,//
0x2e,0x80,
0x3c,0x00,
0x3D,0x20, //
0xfe,0x00,
#else //for test
//0x03,0x03,
//0x04,0xff,
#endif

	/////////////////////////////////////////////////
	/////////////////////  DVP   ////////////////////
	/////////////////////////////////////////////////
#if 1
0xfe,0x03,
0x01,0x00,
0x02,0x00,
0x10,0x00,
0x15,0x00,
0xfe,0x00,
#endif
	///////////////////OUTPUT//////////////////////
0xf3,0xff, // output_enable
-1,-1
};
#endif 

#if 0	//Lunata X-SJ-1000WL-V02(senser0312)
SENSOR_INIT_SECTION static const unsigned char GC0312InitTable[CMOS_INIT_LEN]=
{

	//Lunata X-SJ-1000WL-V02(senser0312)
	//0xF1,0x10,
	0xFE,0xF0,
	0xFE,0xF0, 
	0xFE,0x00, 
	0xFC,0x0E, 
	0xFC,0x0E, 
	
	0xF2,0x07, 	
	0xF3,0x00, 
	0xF7,0x1B, 
	0xF8,0x04,	 
	0xF9,0x0E, 
	0xFA,0x11, 
	
	0x00,0x2F, 
	0x01,0x0F,
	0x02,0x04, 
	0x03,0x03, 
	0x04,0x50, 

	0x09,0x00, 
	0x0A,0x00, 
	0x0B,0x00, 
	0x0C,0x04, 
	0x0D,0x01, 
	0x0E,0xE8, 
	
	0x0F,0x02, 
	0x10,0x88, 
	0x16,0x00, 
	0x17,0x14, //0x15->0x14
	0x18,0x1A, 
	0x19,0x14, 
	
	0x1B,0x48, 
	
	0x1E,0x6B, 
	0x1F,0x28, 
	0x20,0x8B, 
	0x21,0x49, 
	0x22,0xB0, 
	0x23,0x04, 
	0x24,0x16, 
	0x34,0x20, 
	
	0x26,0x23, 
	0x28,0xFF, 
	0x29,0x00, 
	0x33,0x10, 
	0x37,0x20, 
	0x38,0x10, 
	0x47,0x80, 
	0x4E,0x66, 
	0xA8,0x02, 
	0xA9,0x80, 
	
	0x40,0xFF, 	
	0x41,0x21, 
	0x42,0x4D, 
	0x44,0x02, 
	0x45,0xA8, 
	0x46,0x02, 	//0x03->0x02
	0x4A,0x11, 
	0x4B,0x01, 
	0x4C,0x20, 
	0x4D,0x05, 
	0x4F,0x01, 

	0x50,0x01, 
//	0x51,0x00, 	//BK->TX
//	0x52,0x00, 
//	0x53,0x00, 
//	0x54,0x50, 
	0x55,0x01, 
	
	0x56,0xE0, 
	0x57,0x02, //0X01->0X02
	0x58,0x80, //0XE0->0X80*/
	
	,
	
	-1,-1
};

#endif

#if 1//def I4_DEV_ICM_003
SENSOR_INIT_SECTION static const unsigned char GC0312InitTable[CMOS_INIT_LEN]=
{
	//X8-GC0312
	//ypc 20240117
	
	,
	-1,-1

};

#endif

extern struct i2c_device *iic_test;

static void GC0312_rotate(uint32 r)
{
	unsigned char buf[2];
	buf[0] = 0x14;		//0',180'
	buf[1] = 0x10|(r<<0);
	//Sensor_WriteRegister(SENSOR_IIC,buf,1,1);
	//i2c_sensor_write(iic_test,buf,1,1,0x42,0x43);
	i2c_write(iic_test,(int8 *)buf,1,(int8 *)&buf[1],1);

}



uint32 GC0312_hvblank(int8 dh,int8 dv)
{
#define HBLANK_BASE	0xdeUL
#define VBLANK_BASE	0x24UL
		uint32 i;
		int32 h = (int32)HBLANK_BASE + dh; if(h < 0) h = 0;
		int32 v = (int32)VBLANK_BASE + dv; if(v < 0) v = 0;
		const uint8 t[][2] = {
			{0x01,h & 0xff},//0x6a//0xde //HB[7:0]
			{0x02,v & 0xff},  //0x0c //VB[7:0]
			{0x0f,((v >> 8)<<4)|(h >> 8)} //{0x0f,0x00}, //VB/HB [11:8]
		};
	//for(i=0;i<3;i++)
		//Sensor_WriteRegister(SENSOR_IIC,(u8 *)&t[i][0],1,1);
	for(i=0;i<3;i++)	
		i2c_write(iic_test,(int8 *)&t[i][0],1,(int8 *)&t[i][1],1);

	return (h<<16)|v;
}

SENSOR_OP_SECTION const _Sensor_Adpt_ gc0312_cmd= 
{
	.typ = 1, //YUV
	#ifdef I4_DEV_ICM_003
	.pixelw = 480,
	#else
	.pixelw = 640,
	#endif
	.pixelh= 480,
	.hsyn = 1,
	.vsyn = 0,		//0312：0
	.rduline = 0,//
	.rawwide = 1,//10bit
	.colrarray = 2,//0:_RGRG_ 1:_GRGR_,2:_BGBG_,3:_GBGB_
	.init = (uint8 *)GC0312InitTable,
	.rotate_adapt = {0},
	//.hvb_adapt = {0xde,0x24,0xde,0x24},
	. mclk = 24000000,
	
	.p_fun_adapt = {GC0312_rotate,GC0312_hvblank,NULL},		//0312
};

const _Sensor_Ident_ gc0312_init=
{
	0x10,0x42,0x43,0x01,0x01,0xf1 		//0312 0x21>>w_0x42,r_0x43
};



#endif


